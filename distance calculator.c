...
int main(){
	...
	for(i=0;i<4;i++){
		if(i%2==0){
			...
			while(hemisphere[i]!='n'&&hemisphere[i]!='s'||(coord[i]<0||coord[i]>90));
		}
		else{
			do{
	...
	if(round(difference*10000)/10000!=round(dist_angle*10000)/10000&&round(sin(difference)*10000)/10000!=0){
		angle_a=asin(sin(difference)/sin(dist_angle)*sin(colatt[1]));
		angle_b=asin(sin(difference)/sin(dist_angle)*sin(colatt[0]));
	}
	else{
		angle_a=asin(sin(colatt[1]));
		angle_b=asin(sin(colatt[0]));
		circular_tri=1;
	}
	printf("\nGreat circle distance = %.4lf (km)\n",6370*dist_angle);
	if((angle_a+angle_b+difference)/pi*180-180<0||round(sin(difference)*10000)/10000!=0){
	...
	if(circular_tri==0){
		...
		printf("Spherical triangle is circular\n");
	}
	...
	if(yesorno=='y'){
		...
		if(circular_tri==0){
			...
			printf("Spherical excess = %.4lf + %.4lf + %.4lf - 180 = %.4lf\n"
			,degree(angle_a),degree(angle_b),degree(difference),degree(angle_a+angle_b+difference)-180);
		}
		else{
			printf("Since c = 180, spherical triangle is circular\n");
		}
		delay(300);
	}
